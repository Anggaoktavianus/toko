...
class Items extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->model(['m_items', 'm_category', 'm_unit']);
    }
    ...
    function get_ajax()
    {
        ...
        foreach ($list as $item) {
            ...
            $row = array();
            $row[] = $no . ".";
            $row[] = '<a href="' . site_url('items/barcode_qrcode/' . $item->id_item) . '" class="btn btn-default btn-sm">' . $item->barcode . '</a>';
            $row[] = $item->name;
            ...
            // add html for action
            $row[] = '<a href="' . site_url('items/edit/' . $item->id_item) . '" class="btn btn-success text-white"><i class="fas fa-pencil-alt"></i></a>
                    <a href="' . site_url('items/del/' . $item->id_item) . '" onclick="return confirm(\'Yakin hapus data?\')"  class="btn btn-danger text-white"><i class="fas fa-trash-alt"></i></a>';
            $data[] = $row;
        }
        $output = array(
            "draw" => @$_POST['draw'],
