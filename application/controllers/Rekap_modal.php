        ...
        if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter dan klik tombol tampilkan
            ...
            if($filter == '1'){ // Jika filter nya 1 (per tanggal)
                ...
                if (!empty($created_at) && !empty($created_at1) && !empty($pegawai)) {
                        ...
                        $url_export = 'rekap_modal/export?filter=1&tanggal&tanggal1='.$created_at.$created_at1.$userid;
                        ...
                        }else if (!empty($created_at) && !empty($created_at1)){
                        $label = 'Data Transaksi Tanggal '.date('d-m-y', strtotime($created_at)).'&nbsp;'.'Sampai'.$created_at1;
                        $total = $this->m_rekap->view_by_date_id($created_at,$created_at1,$userid);
                        $url_export = 'rekap_modal/export?filter=1&tanggal='.$created_at.$userid;
                        $transaksi = $this->m_rekap->view_by_date($created_at, $created_at1,$userid);     
                        } else if (!empty($created_at)){
                        $label = 'Data Transaksi Tanggal '.date('d-m-y', strtotime($created_at)).'&nbsp;'.'oleh'.$userid;
                        $total = $this->m_rekap->view_by_date_id($created_at,$created_at1,$userid);
                        $url_export = 'rekap_modal/export?filter=1&tanggal='.$created_at.$userid;
                        $transaksi = $this->m_rekap->view_by_date($created_at, $created_at1,$userid); 
                ...
                $url_export = 'rekap_modal/export?filter=2&bulan='.$pegawai;
            }
        }else{ // Jika user tidak mengklik tombol tampilkan
            $label = 'Semua Data Transaksi';
